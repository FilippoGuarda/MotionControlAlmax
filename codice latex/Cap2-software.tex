...
\primalettera{I}l modello di motion control è basato su ROS, un software
di connessione, che facilita la comunicazione tra diverse macchine
sulle quali è installato; nel nostro caso, tra la base station e il
...
In questo capitolo si parlerà, nello specifico, di ROS e CANopen over
CAN.

\section{ROS}

ROS, acronimo di Robot Operative System, non è un sistema operativo
in sè, ma un middleware open-source per la robotica. \cite{ROS2009}
...
Lo sviluppo è basato su un modello \textquotedbl{}Federale\textquotedbl{}
tale che un gruppo può scaricare il codice di ROS nel proprio server,
...
svolgimento di funzioni per e da altri nodi nonchè accedere a un database
...
del rover; attraverso i topics è possibile mettere in comunicazione
...
Il nodo più importante nello schema di ROS è il master; esso infatti
...
a vicenda quando hanno la necessità di scambiare informazioni; svolge
...
La pubblicazione e la sottoscrizione ad un topic è anonima, sicchè
...
esterna con più potenza di calcolo. 
...
più processi di inizializzazione in uno script, in modo da ridurli
...
CAN è un bus seriale producer-consumer nel quale i vari nodi sono
...
una tensione più alta di CAN-L. Quando non viene più fornita tensione
vengono riportati in uno stato recessivo da un resistore passivo,
con CAN-H a tensione più bassa di CAN-L. Un bit a 0 codifica un segnale
...
(un parametro statico), fino ad arrivare al target di velocità nel
...
si accede tramite un indice esadecimale, a cui è aggiunto un sotto-indice
...
segue una risposta. Quindi, per leggere 4 bytes di informazione è
...
Il PDO è un servizio a basso overhead (solo 4 bytes), molto più veloce
...
è la modalità a target di velocità. Sono specificati inoltre un set
...
al drive (gestione errori). Lo stato attuale è indicato da una status-word
...
e altri comandi, tra cui il target di velocità, vengono inviati attraverso
